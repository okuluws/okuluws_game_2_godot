...
@onready var Client = $"/root/Main/Client"
...
#signal player_joined(user_id, profile_id)
#signal player_left(user_id, profile_id)
#
#
...
#func _on_client_disconnected(peer_id):
	...
	#FuncU.map_dict(players, func(user_id, data):
		#if data.peer_id == peer_id:
			#players.erase(user_id)
			#player_left.emit(user_id, data.profile_record_id)
	#)
#
#
#
...
#func connect_player(peer_id: int, authtoken: String, profile_id: String) -> void:
	...
	#player_joined.emit(user.id, profile.id)
#
#
#@rpc("any_peer")
...
#func _on_print_profiles_pressed():
	#assert(multiplayer.is_server())
	#print(Inventories.server_records)
#
#
