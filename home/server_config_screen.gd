...
func _ready():
	var err = game_main.func_u.ConfigFile_load(c, world_dir_path.path_join("world.cfg"))
	if err != null:
		push_error(err)
		queue_free()
		return
...
func _on_btn_back_arrow_pressed():
	var err = game_main.func_u.ConfigFile_save(c, world_dir_path.path_join("world.cfg"))
