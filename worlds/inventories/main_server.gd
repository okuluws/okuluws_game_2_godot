...
var items_config: GameMain.Worlds.Server.Items.Config
var inventories = {}


func init(p_server: GameMain.Worlds.Server) -> void:
	server = p_server
	savefile = server.world_dirpath.path_join("inventories.cfg")
	items_config = server.items.config
	if not FileAccess.file_exists(savefile): FileAccess.open(savefile, FileAccess.WRITE)
	server.saving.connect(_save_inventories)
	_load_inventories()
